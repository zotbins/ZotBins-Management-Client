...
class IntervalOption extends React.Component {
  constructor(props) {
    ...
    this.menu = (
      <Menu onClick={this.handleMenuClick}>
        <Menu.Item key="1">Last 6 Months</Menu.Item>
        <Menu.Item key="2">Last Month</Menu.Item>
        <Menu.Item key="3">Last Week</Menu.Item>
        <Menu.Item key="4">Last Hour</Menu.Item>
      </Menu>
    )
  }
  ...
  handleMenuClick(e) {
    ...
    if (e.key == 1) {
      this.props.updateDates([
        new Date(Date.now() - 15552000000 - 28800000).toISOString(),
        new Date(Date.now() - 28800000).toISOString(),
      ])
    }
    if (e.key == 2) {
      this.props.updateDates([
        new Date(Date.now() - 2592000000 - 28800000).toISOString(),
        new Date(Date.now() - 28800000).toISOString(),
      ])
    }
    if (e.key == 3) {
      this.props.updateDates([
        new Date(Date.now() - 604800000 - 28800000).toISOString(),
        new Date(Date.now() - 28800000).toISOString(),
      ])
    }
    if (e.key == 4) {
  ...
  onOk(value) {
    ...
    this.props.updateDates([
      new Date(value[0]._d - 28800000).toISOString(),
      new Date(value[1]._d - 28800000).toISOString(),
    ])
  }
